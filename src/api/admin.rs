...
async fn has_http_access() -> bool {
    ...
    match http_access.head("https://github.com/dpinse/vaultwarden").send().await {
...
async fn get_release_info(has_http_access: bool, running_within_docker: bool) -> (String, String, String) {
    ...
    if has_http_access {
        (
            match get_json_api::<GitRelease>("https://api.github.com/repos/dpinse/vaultwarden/releases/latest")
                .await
            {
                Ok(r) => r.tag_name,
                _ => "-".to_string(),
            },
            match get_json_api::<GitCommit>("https://api.github.com/repos/dpinse/vaultwarden/commits/main").await {
                Ok(mut c) => {
                    c.sha.truncate(8);
                    c.sha
                }
            ...
            if running_within_docker {
                ...
                match get_json_api::<GitRelease>(
                    "https://api.github.com/repos/dpinse/bw_web_builds/releases/latest",
                )
                .await
                {
